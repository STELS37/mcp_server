...
## Обязательный skill для подписываемых PDF

Любой агент/менеджер, который создаёт или изменяет банковский, договорный, бухгалтерский, кредитный, юридический или иной подписываемый PDF, **обязан до начала работы прочитать и применить**:

`docs/skills/signed-pdf-document-gate/SKILL.md`

Это blocking gate для Bank Manager, Office Managers, Sales Managers и любых других воркеров, формирующих внешние подписываемые документы.

Документ нельзя считать готовым и нельзя отправлять наружу, пока не пройдены все инварианты skill, включая:
- точное семантическое попадание данных в строки/столбцы/поля;
- сохранение исходной геометрии и границ страниц;
- обязательную проверку каждого листа после рендера;
- независимую привязку даты, рукописного ФИО, подписи и печати;
- корректный физический размер печати/подписи;
- соблюдение требований получателя к разрешённому способу подписи;
- сохранение одностраничных согласий на одном листе, если исходная форма одностраничная;
- entity/INN guard всех вложений;
- freshness rule для повторно запрошенных банковских документов;
- raster/flatten финальной подписанной копии, если получателю нужен скан;
- повторный visual QA уже flattened-файла;
- read-back фактических Gmail-вложений после отправки.

Наличие хотя бы одного визуального/геометрического дефекта означает `NOT READY`, а не «почти готово».
